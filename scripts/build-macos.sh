...
# with "Apple Development: ..." — a development certificate. It passed every
# local check and then refused to open on the first user's Mac ("cannot be
# opened because it is from an unidentified developer"), because a development
# certificate is not a distribution one. The identity comes from the environment
# here, and the script refuses to produce a release build with anything else.
#
...
#      reads as a corrupt download. So the dmg is rebuilt around the stapled
#      app, then submitted and stapled here, after the bundler finishes.
#
...
#                           pass, which cannot read a keychain profile. Note the
#                           variable is APPLE_PASSWORD — not electron-builder's
...
case "$IDENTITY" in
  ...
  *)
    ...
    echo "A development certificate signs cleanly and is still refused by Gatekeeper." >&2
    exit 1
    ;;
esac
...
# The Tauri CLI resolves the project by walking down for tauri.conf.json, so it
# runs from the repo root — not from src-tauri.
cd "$REPO_ROOT"
"$TAURI" "${BUILD_ARGS[@]}"

TARGET_DIR="$REPO_ROOT/target"
[ -n "$TARGET" ] && TARGET_DIR="$TARGET_DIR/$TARGET"
APP="$TARGET_DIR/release/bundle/macos/$PRODUCT.app"
DMG="$(/usr/bin/find "$TARGET_DIR/release/bundle/dmg" -name "*.dmg" -print -quit 2>/dev/null || true)"

[ -d "$APP" ] || { echo "No $PRODUCT.app was produced." >&2; exit 1; }
